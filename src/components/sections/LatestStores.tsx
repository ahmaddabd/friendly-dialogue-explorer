...
import { Store, ShoppingBag, Star, MapPin } from "lucide-react";
      ...
      <div className="container mx-auto px-4">
        ...
        >
          <h2 className="text-3xl font-bold text-gray-900 mb-4">
            {lang === "ar" ? "أحدث المتاجر" : "Latest Stores"}
          </h2>
          <p className="text-gray-600 max-w-2xl mx-auto">
            {lang === "ar"
              ? "اكتشف أحدث المتاجر المنضمة إلى منصتنا"
              : "Discover the newest stores that joined our platform"}
          </p>
        </motion.div>
                ...
                <Card className="group hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2 overflow-hidden bg-white/80 backdrop-blur-sm border-green-100/20">
                  <div className="p-6">
                    <div className="relative">
                      {/* Store Logo */}
                      <div className="w-16 h-16 bg-green-100 rounded-lg mb-4 overflow-hidden">
                        {store.logo_url ? (
                          <img
                            src={store.logo_url}
                            alt={store.name}
